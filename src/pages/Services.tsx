  ...
  return (
    <div className={`min-h-screen bg-white text-zinc-800 flex flex-col justify-between font-sans overflow-x-hidden ${isRtl ? 'text-right' : 'text-left'}`}>
      ...
      <main className="flex-grow pt-[104px]">
            ...
            <div className="grid grid-cols-1 lg:grid-cols-2 gap-8">
              {data.servicesList.map((service) => (
                <div 
                  key={service.id}
                  id={service.id}
                  className="bg-white border border-zinc-200/70 rounded-3xl p-8 transition-all duration-300 hover:border-brand-yellow hover:shadow-lg flex flex-col justify-between relative overflow-hidden group"
                >
                  ...
                  <div className="space-y-6">
                    <div className="flex items-center gap-3">
                      <span className="text-xs font-black text-brand-yellow bg-brand-dark px-3 py-1 rounded-lg">
                        {service.category}
                      </span>
                    </div>
                    ...
                    <div className="grid grid-cols-1 sm:grid-cols-2 gap-3 pt-4 border-t border-zinc-100">
                      {service.specs.map((spec, i) => (
                        <div key={i} className={`flex items-center gap-2.5 ${isRtl ? 'flex-row-reverse text-right' : 'text-left'}`}>
                          <CheckCircle className="h-4 w-4 text-brand-yellow shrink-0" />
                          <span className="text-xs font-bold text-zinc-700">{spec}</span>
                        </div>
                      ))}
                    </div>
                  </div>
                  ...
                  <a
                    href="#quote-form"
                    className="inline-flex items-center gap-1.5 mt-8 text-xs font-black uppercase tracking-widest text-brand-dark hover:text-brand-yellow transition-all w-fit cursor-pointer"
                  >
        ...
        <section className="py-24 bg-brand-dark text-white border-b border-zinc-950">
          <div className="max-w-7xl mx-auto px-6">
            
            <div className="text-center max-w-2xl mx-auto mb-20 space-y-4">
              <span className="text-xs font-black uppercase tracking-widest text-brand-yellow bg-zinc-900 border border-zinc-850 px-3.5 py-1.5 rounded-lg">
            ...
            <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-4 gap-8">
              {data.process.steps.map((step, index) => (
                <div 
                  key={index}
                  className="bg-zinc-950/40 border border-zinc-900 rounded-3xl p-8 relative overflow-hidden group hover:border-brand-yellow transition-all duration-300"
                >
                  <div className="absolute right-4 top-4 text-4xl md:text-5xl font-black text-zinc-850/50 group-hover:text-brand-yellow/15 transition-all">
                    {step.number}
                  </div>
                  <span className="text-2xl font-black text-brand-yellow block mb-4">
                    {step.number}
                  </span>
                  <h3 className="text-base font-black text-white tracking-tight">
                    {step.title}
                  </h3>
                  <p className="text-xs text-zinc-400 font-semibold mt-3.5 leading-relaxed">
                    {step.description}
                  </p>
                </div>
              ))}
        ...
        <section id="quote-form" className="py-24 bg-white">
          <div className="max-w-3xl mx-auto px-6">
            <div className="bg-zinc-950 text-white rounded-3xl p-8 md:p-12 border border-zinc-900 shadow-2xl relative overflow-hidden">
              <div className="absolute -right-20 -bottom-20 w-64 h-64 bg-brand-yellow/5 rounded-full blur-3xl" />
                ...
                <form onSubmit={handleSubmit} className="space-y-6">
                  <div className="grid grid-cols-1 sm:grid-cols-2 gap-6">
                    ...
                    <div className="flex flex-col gap-1.5">
                      ...
                      <input
                        ...
                        className="px-4 py-3.5 bg-zinc-900 border border-zinc-800 focus:border-brand-yellow rounded-xl focus:outline-none text-xs font-semibold text-white"
                      />
                    </div>
                  ...
                  <div className="flex flex-col gap-1.5">
                    ...
                    <textarea
                      ...
                      onChange={handleInputChange}
                      className="px-4 py-3.5 bg-zinc-900 border border-zinc-800 focus:border-brand-yellow rounded-xl focus:outline-none text-xs font-semibold text-white resize-none"
                    />
                  </div>
